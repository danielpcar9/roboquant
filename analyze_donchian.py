...
def analyze_donchian_structure():
    ...
    try:
        ...
        print(f"Líneas de código: {len([l for l in lines if l.strip() and not l.strip().startswith('#')])}")
        print(f"Líneas en blanco: {len([l for l in lines if not l.strip()])}")
        print(f"Comentarios: {len([l for l in lines if l.strip().startswith('#')])}")
        ...
        for section, line_numbers in sections_found.items():
            print(f"{section.replace('_', ' ').title()}: {len(line_numbers)} líneas encontradas")
