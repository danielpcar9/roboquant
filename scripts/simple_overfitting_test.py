...
def walk_forward_test(df, n_windows=5):
    ...
    for i in range(n_windows):
        ...
        logging.info(f"  IS:  Return={is_result['total_return']:.1f}%, Trades={is_result['total_trades']}, WinRate={is_result['win_rate']:.1f}%")
        logging.info(f"  OOS: Return={oos_result['total_return']:.1f}%, Trades={oos_result['total_trades']}, WinRate={oos_result['win_rate']:.1f}%")
    
    results_df = pd.DataFrame(results)
    
    # Calculate degradation
    results_df['Degradation_%'] = results_df.apply(
        lambda row: ((row['IS_Return'] - row['OOS_Return']) / abs(row['IS_Return']) * 100) if row['IS_Return'] != 0 else 0,
        axis=1
    )
...
def parameter_robustness_test(df):
    ...
    for period in periods:
        ...
        logging.info(f"  Return={result['total_return']:.1f}%, Trades={result['total_trades']}, WinRate={result['win_rate']:.1f}%")
...
def main():
    ...
    print("="*70)
    rob_results = parameter_robustness_test(df)
    
    print("\n\nTest 3: Yearly Consistency")
    print("="*70)
    yearly_results = yearly_consistency_test(df)
