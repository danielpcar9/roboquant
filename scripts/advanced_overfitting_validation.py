...
def backtest_with_regime_filter(df, donchian_period=20, use_adx_filter=True, adx_threshold=18, di_threshold=26, random_seed=None):
    """
    ...
    tr = pd.concat([df['high'] - df['low'], np.abs(df['high'] - df['close'].shift()), np.abs(df['low'] - df['close'].shift())], axis=1).max(axis=1)
    atr = tr.rolling(window=14).mean()
    df['plus_di'] = 100 * (pd.Series(plus_dm, index=df.index).rolling(window=14).mean() / atr)
    df['minus_di'] = 100 * (pd.Series(minus_dm, index=df.index).rolling(window=14).mean() / atr)
    ...
    if use_adx_filter:
        ...
        df['long_signal'] = df['long_signal'] & (df['adx'] > adx_threshold) & (strength >= di_threshold)
        df['short_signal'] = df['short_signal'] & (df['adx'] > adx_threshold) & (strength >= di_threshold)
    ...
    sharpe = (returns.mean() / returns.std() * np.sqrt(252 * 24)) if returns.std() > 0 else 0  # Hourly data
...
def anchored_walk_forward(df, initial_train_years=2, test_months=6):
    ...
    while train_end < df.index[-1]:
        ...
        logging.info(f"  Train (No Filter): Return={train_result['total_return']:.1f}%, Sharpe={train_result['sharpe_ratio']:.2f}")
        logging.info(f"  Test (No Filter):  Return={test_result['total_return']:.1f}%, Sharpe={test_result['sharpe_ratio']:.2f}")
        logging.info(f"  Train (ADX+DI):    Return={train_result_adx['total_return']:.1f}%, Sharpe={train_result_adx['sharpe_ratio']:.2f}")
        logging.info(f"  Test (ADX+DI):     Return={test_result_adx['total_return']:.1f}%, Sharpe={test_result_adx['sharpe_ratio']:.2f}")
...
def rolling_walk_forward(df, train_years=3, test_months=6):
    ...
    while True:
        ...
        logging.info(f"  Test (No Filter): Return={test_result['total_return']:.1f}%, Sharpe={test_result['sharpe_ratio']:.2f}, Trades={test_result['total_trades']}")
        logging.info(f"  Test (ADX+DI):    Return={test_result_adx['total_return']:.1f}%, Sharpe={test_result_adx['sharpe_ratio']:.2f}, Trades={test_result_adx['total_trades']}")
    ...
    if len(trending_results) > 0:
        print(f"  Avg Return (No Filter): {trending_results['Test_Return_NoFilter'].mean():.2f}%")
        print(f"  Avg Return (ADX+DI): {trending_results['Test_Return_ADX'].mean():.2f}%")
        print(f"  Avg Sharpe (ADX+DI): {trending_results['Test_Sharpe_ADX'].mean():.2f}")
    
    print(f"\nRANGING Markets ({len(ranging_results)} windows):")
    if len(ranging_results) > 0:
        print(f"  Avg Return (No Filter): {ranging_results['Test_Return_NoFilter'].mean():.2f}%")
        print(f"  Avg Return (ADX+DI): {ranging_results['Test_Return_ADX'].mean():.2f}%")
...
def random_seed_stress_test(df, n_iterations=20):
    ...
    for i, seed in enumerate(seeds):
        ...
        result = backtest_with_regime_filter(df, use_adx_filter=True, random_seed=seed, adx_threshold=18, di_threshold=26)
        ...
        logging.info(f"  Return: {result['total_return']:.2f}%, Sharpe: {result['sharpe_ratio']:.2f}, Trades: {result['total_trades']}")
    ...
    if sharpe_std < 0.25:
        ...
        print(f"  Strategy is highly stable across random initializations")
    elif sharpe_std < 0.4:
        print(f"  ROBUST (Sharpe Std: {sharpe_std:.3f} < 0.40)")
        print(f"  Strategy shows good stability")
    else:
        print(f"  OVERFIT WARNING (Sharpe Std: {sharpe_std:.3f} > 0.40)")
        print(f"  Strategy may be overfitted - high sensitivity to random variations")
...
def main():
    ...
    print("="*70)
    anchored_results = anchored_walk_forward(df, initial_train_years=2, test_months=6)
    ...
    trending_positive = len(rolling_results[(rolling_results['Regime'] == 'TRENDING') & (rolling_results['Test_Return_ADX'] > 0)])
    ranging_positive = len(rolling_results[(rolling_results['Regime'] == 'RANGING') & (rolling_results['Test_Return_ADX'] > 0)])
    
    trending_success_rate = (trending_positive / trending_count * 100) if trending_count > 0 else 0
    ranging_success_rate = (ranging_positive / ranging_count * 100) if ranging_count > 0 else 0
    
    avg_sharpe = rolling_results['Test_Sharpe_ADX'].mean()
    
    print(f"\n1. REGIME PERFORMANCE:")
    print(f"   Trending Markets: {trending_positive}/{trending_count} positive ({trending_success_rate:.0f}%)")
    print(f"   Ranging Markets: {ranging_positive}/{ranging_count} positive ({ranging_success_rate:.0f}%)")
    ...
    if trending_success_rate < 60:
        ready_for_live = False
        issues.append(f"Low trending market success rate ({trending_success_rate:.0f}%)")
    ...
    if ready_for_live and len(issues) == 0:
        ...
        print(f"  - Regime Success: {trending_positive + ranging_positive}/{total_regimes} ({(trending_positive + ranging_positive)/total_regimes*100:.0f}%)")
