    ...
    def _generate_signal(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if upper_channel is None or lower_channel is None:
                return {'should_enter': False, 'reason': 'Failed to calculate Donchian channels'}
            ...
            if buy_signal:
                return {'should_enter': True, 'direction': 'BUY', 'reason': 'Price above upper channel'}
            elif sell_signal:
                return {'should_enter': True, 'direction': 'SELL', 'reason': 'Price below lower channel'}
            else:
                return {'should_enter': False, 'reason': 'No breakout signal'}

        except Exception as e:
            logging.error(f"Signal generation error: {e}")
            return {'should_enter': False, 'reason': f'Signal generation failed: {str(e)}'}
