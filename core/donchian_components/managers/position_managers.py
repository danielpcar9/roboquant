...
from core.brokers.mt5_utils import build_and_send_order, estimate_lots_by_risk
from core.news.news_filter import news_filter
from core.analysis.market_regime import market_regime_detector
...
class PositionManager:
    ...
    def __init__(self, market_data_service: TechnicalIndicatorsCalculator, risk_validator: RiskValidator):
    ...
    def validate_trading_hours(self) -> bool:
        ...
        logging.debug(f"México: {current_hour_local}:00 | UTC: {current_hour_utc}:00 | Trading: {trading_start}-{trading_end} UTC | Active: {in_hours}")

        return in_hours
    
    @handle_exception
    def execute_trade(self, symbol: str, order_type: str, lots: float, sl_points: float, tp_points: float) -> bool:
    ...
    def validate_market_conditions(self, symbol: str) -> tuple[bool, str]:
        ...
        if spread > self.config['max_spread_points']:
            return False, f"Spread too high: {spread:.2f} points > {self.config['max_spread_points']} points"
        ...
        if require_adx:
            ...
            if regime == "RANGING":
                return False, f"Market is RANGING (ADX: {adx_value:.2f}), skipping trade"
            elif regime == "UNKNOWN":
...
class TradeTracker:
    ...
    @handle_exception
    def track_new_trade(self, ticket: int, symbol: str, order_type: str, entry_price: float, sl: float, tp: float):
        ...
        logging.info(f"Tracking new trade: Ticket {ticket} for {symbol} at {entry_price}")
    ...
    def get_active_trades_count(self, symbol: Optional[str] = None) -> int:
        ...
        if symbol:
            return len([t for t in self.active_trades.values() if t['symbol'] == symbol])
