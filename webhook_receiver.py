    ...
    def process_trade_signal(self, signal_data):
        ...
        try:
            ...
            if sl_points == 0 or tp_points == 0:
                ...
                atr = calculate_atr(symbol) if symbol_info else 5.0
                sl_multiplier = 3.0
                tp_multiplier = 6.0
...
def webhook():
    ...
    try:
        ...
        if not is_valid:
            logging.warning(f"Signature verification failed from {request.remote_addr}: {error_msg}")
            return jsonify({'error': error_msg}), 401 if error_msg != 'Server not configured securely' else 500
